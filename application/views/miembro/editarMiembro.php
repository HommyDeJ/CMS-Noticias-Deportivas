<?php
        ...
        $id = $_GET['id'];
      		...
      		$CI->db->where('id', $id);
      		$filas = $CI->db->get('miembro')->result_array();

      		if (count($filas) > 0) {
      			$fila = $filas[0];
      		}else{
      			$fila = array('id'=>'','cedula'=>'','nombre'=>'','telefono'=>'', 'email'=>'', 'celular'=>'', 'direccion'=>'', 'latitud'=>'', 'longitud'=>'', 'contrasena'=>'',);
      		}
...
<form action="<?php echo base_url('miembro/registrar')?>" method="post" enctype="multipart/form-data">
  <div class="col col-md-12">
    <div>
      <span><input name="id" value="<?php echo $fila['id']?>" type="hidden" class="form-control"></span>
    </div>
    ...
    <div>
      ...
      <span><input required name="nombre" value="<?php echo $fila['nombre']?>" type="text" class="form-control"></span>
    </div>

    <div>
      <span><label>Telefono</label></span>
      <span><input name="telefono" value="<?php echo $fila['telefono']?>" type="text" class="form-control"></span>
    </div>

    <div>
      <span><label>Email</label></span>
      <span><input required name="correo" value="<?php echo $fila['email']?>" type="text" class="form-control"></span>
    </div>

    <div>
      <span><label>Contraseña</label></span>
      <span><input required name="contrasena" value="<?php echo $fila['contrasena']?>" type="password" class="form-control"></span>
    </div>
    ...
    <div>
      ...
      <span><input name="celular" type="text" value="<?php echo $fila['celular']?>" class="form-control"></span>
    </div>
    ...
    <div>
      ...
      <span><input required name="direccion" value="<?php echo $fila['direccion']?>" type="text" class="form-control"></span>
    </div>
    ...
    <div>
      ...
      <span><input name="latitud" id="latitud" type="text" readonly value="<?php echo $fila['latitud']?>" class="form-control"></span>
    </div>

    <div>
      <span><label>Longitud</label></span>
      <span><input name="longitud" id="longitud"type="text" readonly value="<?php echo $fila['longitud']?>" class="form-control"></span>
    </div>
    <br/>
